/*
Data dig
Skills used: MAX, MIX, HAVING, CASE, GROUP BY
*/
...
-- We select the titles with the most and least world revenue 
SELECT Title, MAX(worldwide)
FROM topmovies;

SELECT Title, MIN(worldwide)
FROM topmovies;

-- We group by year and show the years and each year sum worldwide revenue 
SELECT year, ROUND(SUM(worldwide),2) AS "Yearly Worldwide"
FROM topmovies
GROUP BY year
HAVING "Yearly Worldwide" > 800
ORDER BY "Yearly Worldwide" DESC;

-- Here we categorize the studios by the total revenue generated by each one 
SELECT studio,
    CASE
        WHEN AVG(worldwide) > 900 THEN " EXTRAKICKASS"
        WHEN AVG(worldwide) > 800 THEN "AVERAGE KICKASS"
        ELSE "LOW KICKASS"
        END AS "Kick ass Studio?"
FROM topmovies
GROUP BY studio
ORDER BY AVG(worldwide) DESC;

-- Finally we search for title and year of movies released between 2003 and 2009
SELECT title, year
FROM topmovies
WHERE year <= 2009 AND year >= 2003;
